...
def main():
    ...
    with st.sidebar:
        ...
        # Tone selection
        st.markdown("#### 🎵 Narration Tone")
        tone = st.radio(
            "Narration Tone",
            ["Neutral", "Explanatory"],
            index=0 if st.session_state.tone == "neutral" else 1,
            label_visibility="collapsed"
        )
    ...
    if uploaded_file is not None:
        ...
        if extracted_text:
            ...
            with st.expander("📝 View Extracted Text", expanded=True):
                st.text_area("Text", extracted_text, height=300, label_visibility="collapsed")
            
            # Current settings
            st.info(f"🎯 Settings: {st.session_state.language}, {st.session_state.voice_type} voice, {st.session_state.tone} tone")
